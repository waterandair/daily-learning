...
func (s *routeGuideServer) GetFeature(ctx context.Context, point *routeguide.Point) (*routeguide.Feature, error) {
	...
	return &routeguide.Feature{"", point}, nil
}
```
...
routeguide.RegisterRouteGuideServer(grpcServer, &routeGuideServer{})
...
Once the gRPC *channel* is setup, we need a client *stub* to perform RPCs. We get this using the `NewRouteGuideClient` method provided in the `routeguide` package we generated from our `.proto` file.
...
```go
feature, err := client.GetFeature(ctx, &routeguide.Point{409146138, -746188906})
if err != nil {
        ...
}
```
...
```go
rect := &routeguide.Rectangle{ ... }  // initialize a routeguide.Rectangle
