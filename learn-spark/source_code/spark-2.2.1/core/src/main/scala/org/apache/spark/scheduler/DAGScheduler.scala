...
/**
  * 实现了面向 stage 的调度机制的高层次调度层,他会为每个 job 计算一个 stage 的 DAG(有向无环图),
  * 追踪 RDD 和 stage 的输出是否被物化了(物化就是说,写入了内存或磁盘等地方), 并且寻找一个最少消耗(最优,最小)调度机制来运行 job
  * 它会将 stage 作为 taskset 提交到底层的 TaskScheduleImpl 上, 来再集群上运行它们(task)
  *
  * 除了处理 stage 的 DAG, 它还负责决定运行每个 task 的最佳位置,基于当前的缓存状态,将这些最佳位置提交给底层的 TaskScheduleImpl.
  * 此外,它会处理由于 shuffle 输出文件丢失导致的失败,在这种情况下,旧的 stage 可能就会被重新提交.一个 stage 内部的失败.如果不是由于
  * shuffle 文件丢失所导致的,会被 TaskScheduler 处理, 它会多次重试每一个 task, 直到最后实在不行,才会取消整个stage
  *
...
class DAGScheduler(
  ...
  /**
    * 获取一个 stage 的父 stage
    * 对一个 stage ,如果它的最后一个 rdd 的所有依赖,都是窄依赖,那么就不会创建任何新的 stage
    * 但是,只要发现这个 stage 的 rdd 宽依赖了某个 rdd, 那么就用宽依赖的那个 rdd 创建一个新的 stage
    * 然后立即将新的 stage 返回
    * @param stage
    * @return
    */
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        /**
          * 其实对于每一种有 shuffle 的操作,比如 groupByKey, reduceByKey, countByKey
          * 底层对应了三个 RDD: MapPartitionsRDD, shuffleRDD, MapPartitionsRDD
          */
        if (rddHasUncachedPartitions) {
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              /**
                * 如果是宽依赖
                * 那么使用宽依赖的那个 rdd , 创建一个 stage, 并且会将 isShuffleMap 设置为 true
                * 默认最后一个 stage, 不是shuffleMap stage
                * 但是 finalStage 之前所有的 stage, 都是 shuffleMap stage
                */
    ...
    while (waitingForVisit.nonEmpty) {
      /**
        * 对 stage 的最后一个 rdd , 调用自己内部定义的 visit() 方法
        */
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    /**
      * 使用发出 job 的最后一个 rdd, 创建 finalStage
      */
    ...
    try {
      /**
        * 第一步
        * 创建一个 stage 对象
        * 并且将 state 加入 DAGScheduler 内部的内存缓存中
        */
    ...
    /**
      * 第二步
      * 用 finalStage 创建一个 job
      * 就是说,这个 job 最后一个stage,当然就是finalStage
      */
    ...
    /**
      * 第四步
      * 使用 submitStage() 方法提交 finalStage
      * 这个方法的调用,会导致第一个 stage 提交
      * 并且导致其他所有的 stage 都放入到 waittingStage 队列中
      */
    submitStage(finalStage)

    /**
      * 需要对 stage 划分算法很清晰,当自己编写 spark application 时,应用被划分为了几个 job
      * 每个 job 被划分成了几个 stage,
      * 每个 stage 包括了那些代码
      * 只有知道了每个 stage 包括了那些代码之后,再线上,如果发现某个 stage 执行特别manager,或者一个 stage 一直报错
      * 才能针对那个 stage 对应的代码, 去排查问题
      */

    /**
      * stage 算法总结
      * 1. 从 finalStage 倒腿
      * 2. 通过宽依赖,来进行新的 stage 的划分
      * 3. 使用递归,优先提交父 stage
      */
  }
  ...
  /**
    * 提交 stage
    * 这个就是 stage 划分算法的入口
    * 但是, stage 划分算法,其实是由 submitStage() 方法和 getMissingParentStage() 方法共同组成的
    */
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        /**
          * 调用 getMissingParentStage() 方法,取获取当前这个 stage 的父 stage (根据宽依赖和窄依赖创建stage)
          */
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)

        /**
          * 这里其实会反复递归调用,直到最初的 stage , 它没有父 stage 了
          * 那么,此时,就会取首先提交这个第一个 stage,其余的 stage 此时都在 waitingStages 里面
          */
        if (missing.isEmpty) {
          ...
          /**
            * 发送第一个 stage!!!
            * 为 stage 创建一批 task, task 数量与 partition 数量相同
            */
          submitMissingTasks(stage, jobId.get)
        } else {
          /**
            * 递归调用 submit() 方法, 去调用父 stage
            * 这个递归算法,就是 stage 划分算法的推动者和精髓
            */
          for (parent <- missing) {
            submitStage(parent)
          }
          ...
          /**
            * 并且将当前 stage 放入 waitingStages 等待执行的 statge 的队列中
            */
          waitingStages += stage
        }
      }
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        case stage: ShuffleMapStage =>
          ...
          /**
            * 如果是 ShuffleMapStage
            * 给每一个 Partition 创建一个 task
            * 给每个 task 计算最佳位置
            */
          partitionsToCompute.map { id =>
            val locs = taskIdToLocations(id)  // 最佳位置
            val part = stage.rdd.partitions(id)
            stage.pendingPartitions += id

            /**
              * 这些 ShuffleMapStage 的 isShuffleMap 都是 true 所以会创建 shuffleMapTask
              */
    ...
    if (tasks.size > 0) {
      ...
      /**
        * 最后,针对 stage 的 task, 创建 TaskSet 对象,调用 TaskSchedule 的 submitTasks, 提交 taskSet
        */
  ...
  /**
    * 计算每个 task 对应的 Partition 的最佳位置
    * 就是从 stage 的最后一个 rdd 开始去找,哪个 rdd 的 Partition, 是被 cache了,或者 checkpoint了
    * 那么, task 的最佳位置,就是缓存的/checkpoint 的Partition 的位置
    * 这样的话,task 就在那个节点上执行,不需要计算之前的 rdd 了
    */
  /**
  ...
  private def getPreferredLocsInternal(
    ...
    /**
      * 寻找当前 rdd 的 Partition 是否 checkpoint 了
      */
    ...
    /**
      * 最后,递归调用自己,去寻找 rdd 的父 rdd, 看看对应的 Partition 是否缓存或者checkpoint
      */
    ...
    /**
      * 如果这个 stage, 从最后一个 rdd 到最开始的 rdd, partition 都没有被缓存或者checkpoint, 那么
      * task的最佳位置(preferredLocs), 就是 Nil
      */
    Nil
  }
