...
/**
  * 管理一个 dirver 的执行,包括再 dirver 失败的时候自动重启 dirver,
  * 目前,这种方式仅仅用于 standalone 集群部署模式
  */
/**
    ...
    new Thread("DriverRunner for " + driverId) {
      ...
      override def run() {
        ...
        try {
          ...
          /**
            * 第一步, 创建 driver 的工作目录 并 下载用户上传的 jar (编写完了的 spark 应用程序, 如果是java,用maven 打个 jar 包, 如果是 scala, 那么会用 export 将它到处为jar包)
            */
          // prepare driver jars and run driver
          val exitCode = prepareAndRunDriver()

          /**
            * 对强行杀死 driver 的退出状态做一些处理
            */
        ...
        /**
          * 这个 dirverRunner 这个线程, 向它所属的 worker 的 endpoint(rpc), 发送一个 DriverStateChange 的事件
          * worker 接收到后会发给master, master接收到后会 执行 removeDriver
          */
