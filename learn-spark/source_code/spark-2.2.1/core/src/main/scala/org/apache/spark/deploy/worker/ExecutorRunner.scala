...
private[deploy] class ExecutorRunner(
  ...
  /**
    * worker 中调用 start 方法,启动 executorRunner
    */
  ...
  private def fetchAndRunExecutor() {
    try {
      ...
      /**
        * 重定向输出流到文件
        * 将 executor 的 InputStream 和 ErrorStream 输出的信息分别重定向到本地工作目录的 stdout 文件和 stderr 文件
        */
      ...
      /**
        * 调用 Process 的 waitFor 方法,启动 executor 进程
        */
      ...
      /**
        * executor 执行完后拿到状态的返回值
        */
      state = ExecutorState.EXITED
      val message = "Command exited with code " + exitCode

      /**
        * 向 ExecutorRunner 线程所属的 Worker endpoint(actor), 发送 ExecutorStateChanged消息
        */
