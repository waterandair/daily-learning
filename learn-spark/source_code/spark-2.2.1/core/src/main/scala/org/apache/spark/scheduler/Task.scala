...
private[spark] abstract class Task[T](
  ...
  final def run(
    ...
    /**
      * 创建了一个 TaskContext, 就是 Task 的执行上下文
      * 里面记录了 TaskRUnner 执行的一些去全局性的数据
      * 比如说, task 重试了几次,task 属于哪个 stage , task 要处理的 rdd 的哪个 Partition
      */
  ...
  /**
    * 调用到抽象类,就意味着这个类,只是一个模板类,或者抽象父类,仅仅封装了一些子类通用的数据和操作
    * 而关键的操作,全部要依赖于子类的实现
    * Task 的子类具体有 ShuffleMapTask , ResultTask
    * 要运行它们的 runTask() 方法, 才能执行我们自己定义的算子和逻辑
    */
