...
/**
  * BlockManager 运行在每个节点上,包括 dirver 和 executor, 都会有一份,主要提供了再本地或者远程存储数据的功能
...
private[spark] class BlockManager(
  ...
  /**
    * 每个 BlockManager, 会维护一个 map
    * 其中相当于,在内存中,存放了数据,一个一个的 block 块
    * 是 blockId 到 BlockInfo 的映射
    *
    */
  // Visible for testing
  ...
  /**
    * MemoryStorage 中维护的 entries map , 其实就真的是存放的每个 block 的数据了
    */
  ...
  def initialize(appId: String): Unit = {
    /**
      * 首选初始化, 用于进行远程 block 数据传输的 BlockTransferService
      */
    ...
    /**
      * 为当前这个 BlockManager, 创建一个唯一的 BlockManagerId,
      * 每个 BlockManager 都关联一个 Executor, BlockTransferService 的 hostname,以及 BlockTransferService 的 port
      * 从 BlockManagerId 的初始化即可看出, 一个 BlockManager 是通过一个节点上的 Executor 来唯一标示 的
      */
    val id =
      BlockManagerId(executorId, blockTransferService.hostName, blockTransferService.port, None)

    /**
      * 使用 BlockManagerMaster 的引用,进行 BlockManager 的注册
      */
  ...
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    /**
      * 首先, 执行getLocations(blockId), 会在BlockManagerMaster 上,获取每个blockId 对应的 BlockManager 信息\
      * 然后会随机打乱
      */
    ...
    while (locationIterator.hasNext) {
      ...
      val data = try {
        /**
          * 使用 blockTransferService, 进行异步的远程网络获取,将block数据传输回来
          * 连接的时候,使用的 BlockManager 的唯一标示,就是 host, port, executorId
          */
  ...
  private def doPut[T](
    ...
    /**
      * 为要写入的 block 创建一个 BlockInfo, 并将其放入 blockInfo map 中
      */
