...
private[spark] class Executor(
  ...
  def launchTask(context: ExecutorBackend, taskDescription: TaskDescription): Unit = {
    /**
      * 对于每一个 task, 都会创建一个 TaskRunner
      * TaskRunner 继承的是 java 多线程中的 runable 接口
      */
    ...
    /**
      * java, ThreadPool, Executor 内部有一个 java 线程池
      * 这里其实将 task 封装在一个线程中(taskRunner)
      * 直接将线程丢入线程池,进行执行
      * 所以这里要知道,线程池是自动实现了排队机制的,也就是说,如果线程池内的线程暂时没有空闲的
      * 那么丢进来的线程都是要排队的
      */
  ...
  /**
    * 从 TaskRunner 开始,就是 task 运行的工作原理
    * @param execBackend
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        /**
          * 最后,通过正式的反序列化操作,将整个 task 的数据集反序列化回来
          */
        ...
        /**
          * 这里的 value 对于 shuffleMapTask 来说,就是 MapStatus
          * 封装了 ShuffleMapTask 计算的数据,输出的位置, 如果后面还是一个 ShuffleMapTask 的输出位置,
          * 那么就去联系 MapOutputTracker, 来获取上一个 ShuffleMapTask 的输出位置,然后通过网络拉取数据
          *
          */
        val value = try {
          /**
            * 最关键的部分
            * 执行 task,用的是 Task 的 run 方法
            */
          val res = task.run(
        ...
        /**
          * 这里,是计算出了 task 相关的一些 metrics, 就是统计信息
          * 包括了运行了多长时间,反序列化消耗了多长时间,java虚拟机 gc 消耗了多长时间,结果的序列化耗费了多少时间
          * 这些都可以通过 4040 端口,在 sparkUI 中观察到
          */
        ...
        /**
          * 这个非常核心
          * 调用了 Executor 所在的 GoraseGrainedExecutorBackend 的 statusUpdate() 方法
          */
  ...
  private def updateDependencies(newFiles: Map[String, Long], newJars: Map[String, Long]) {
    ...
    /**
      * 这里用了 java 的 synchronized 进行了多线程并发访问的同步
      * 因为 task 实际上是以 java 线程的方式,在一个 coarseGrainedExecutorBackend 进程内并发运行的
      * 如果在执行业务逻辑的时候,要访问一些共享的资源
      * 那么就有可能出现多线程并发访问的安全问题\
      * 所以 在这里 spark 选择进行了多线程并发访问的同步 (synchronized)
      */
    synchronized {
      ...
      for ((name, timestamp) <- newFiles if currentFiles.getOrElse(name, -1L) < timestamp) {
        ...
        /**
          * 通过 Utils 的 fetchFile() 方法,通过网络通信,从远程拉取文件
          */
