...
/**
  * 1. 底层通过操作一个 SchedulerBackend, 针对不同种类的 cluster (standalone, yarn, mesos) , 调度 task
  * 2. 它也可以通过使用一个 LocalBackend, 并且将 isLocal 参数设置为 true , 来在本地模式下工作
  * 3. 它负责处理一些通用的逻辑,比如说决定多个 job 的调度顺序, 启动推测任务执行
  * 4. 客户端首先调用它的 initialize() 方法 和 start() 方法, 然后通过 runTask() 方法提交 tasksets
  *
  */
/**
...
private[spark] class TaskSchedulerImpl private[scheduler](
  ...
  // 首先调用 initialize 和 start 方法 初始化和启动 taskSchedule
  ...
  /**
    * DAGScheduler 接收到 sparkContext 中 runjob 指令, 创建一个 job,并为 job 划分 stage,
    * 划分完成后, 提交第一个 stage
    * 然后为 stage 创建一批 task,  task 数量与 partition 数量相同,
    * 再 创建 taskset , 调用 TaskSchedule 的 submitTasks, 提交 taskSet
    * @param taskSet
    */
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
      /**
        ...
        * TaskSetManager 实际上负责对应的 taskSet 的任务状况的监视和管理
        */
    ...
    /**
      * 这里的backend 就是初始化操作中针对不同的 spark-submit 提交模式创建的 SchedulerBackend
      * 这个 backend 负责创建 Applient, 向 master 注册 Application 的
      */
  ...
  private def resourceOfferSingleTaskSet(
    ...
    for (i <- 0 until shuffledOffers.size) {
      ...
      /**
        * 如果 当前 executor 的 CPU 数量至少大于 task 要使用的 cpu 数量,默认是 1
        */
      if (availableCpus(i) >= CPUS_PER_TASK) {
        try {
          /**
            * 调用 tasksetManager 的 resourceOffer 方法
            * 去找到, 在这个 executor 上, 就用这种本地化级别,taskset 哪些 task 可以启动
            * 遍历使用当前本地化级别,可以再该executor 上启动的 task
            */
          for (task <- taskSet.resourceOffer(execId, host, maxLocality)) {
            /**
              * 放入 tasks 这个二维数组,给指定的 executor 加上要启动的 task
              */
            tasks(i) += task
            /**
              * 到这里为止,就是 task 分配算法的实现了
              * 尝试着用本地化级别这种模型,去优化 task 的分配和启动,优先希望再最佳本地化的地方启动 task
              * 然后,将 task 分配给 executor
              * 将相应的分配信息加入内存缓存
              */
            val tid = task.taskId
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    /**
      * 首先,将可用的 executor 进行 shuffle, 可就是说,进行打散,从而做到尽量负载均衡
      */
    val shuffledOffers = shuffleOffers(filteredOffers)
    /**
      * 然后针对 WorkerOffer, 创建除一堆需要的东西,比如, tasks,
      */
    // Build a list of tasks to assign to each worker.
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))
    val availableCpus = shuffledOffers.map(o => o.cores).toArray
    /**
      * 这个很重要,从 rootPool 中取出了排序的 TaskSet
      * TaskSchedule 初始化的时候,创建完 TasksSchedulerImpl, SchedulerBackend 之后
      * 执行了一个 initialize() 方法,在这个方法中, 其实会创建一个 调度池
      *
      * 所有提交的 TaskSet 会放入这个调度池,然后执行 task 分配算法的时候,会从这个调度池中,取出排好对的 TaskSet
      */
    ...
    /**
      * 这里是任务分配算法的核心
      *
      * 双重 for 循环,遍历所有 taskset, 以及每一种本地化级别\
      * 对每个 taskset, 从最好的一种本地化级别开始遍历
      */
    ...
    /**
      * 几种本地化级别
      * PROCESS_LOCAL, 进程本地化, rdd 的 Partition 和 task, 进入一个 executor 内,速度非常快
      * NODE_LOCAL, rdd 的 Partition 和 task, 不在一个 executor 内,不在一个进程,但是在一个 worker 节点上
      * NO_PREF, 无,没有所谓的本地化级别
      * RACK_LOCAL, 机架本地化, 至少 rdd 的 Partition 和 task 在一个机架上
      * ANY, 任意的本地化级别
      *
      *
      */
    for (taskSet <- sortedTaskSets) {
      ...
      for (currentMaxLocality <- taskSet.myLocalityLevels) {
        do {
          /**
            * 对当前 taskset
            * 尝试优先使用最小的本地化级别,将taskset 的task ,在 executor 上进行启动
            * 如果启动不了,那么就跳出这个do while 循序, 进入下一个本地化级别,也就是放大本地化级别
            * 以此类推,直到尝试将 taskset 再某些本地化级别下,让task 在 executor 上全部启动
            */
