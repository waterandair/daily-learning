...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    /**
      * 创建 TaskScheduler 和 DAGScheduler
      */
  ...
  /**
    * 首先, hadoopFile() 方法的调用,会创建一个 HadoopRDD, 其中的元素,其实是(key, value) pair
    * key 是 hdfs 或 文本文件的每一行的 offset, value 就是文本行
    * 然后对 HadoopRDD 调用 map() 方法, 会剔除 key, 只保留 value, 然后会获得一个 MapPartitionsRDD
    * MapPartitionsRDD 内部的元素, 其实就是一行一行的文本行
    */
  /**
  ...
  def hadoopFile[K, V](
    ...
    val confBroadcast = broadcast(new SerializableConfiguration(hadoopConfiguration))  // 共享变量
  ...
  def runJob[T, U: ClassTag](
    ...
    /**
      * 调用SparkContext之前初始化创建的 DAGScheduler 的 runJob() 方法
      */
