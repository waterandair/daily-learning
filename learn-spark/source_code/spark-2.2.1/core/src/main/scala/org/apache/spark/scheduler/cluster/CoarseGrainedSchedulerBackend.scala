  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    private def makeOffers() {
      /**
        * 第一步
        *   调用 TaskSchedulerImpl 的 resourceOffers() 方法, 执行任务分配算法,将各个 task 分配到 executor 上去
        * 第二步
        *   分配好 task 到 executor 之后,执行自己的 launchTasks() 方法, 将分配的 task 发送 LaunchTask消息 到
        *   对应的 executor上去, 由 executor启动并执行 task
        *
        * 给 resourceOffers 方法传入的是这个 application 所有可用的 executor,并且将其封装成了 WorkerOffer, 每个 workerOffer
        * 代表了每个 executor 可用的 cpu资源数量
        */
    ...
    /**
      * 根据分配好的情况,去在 executor 上启动相应的 task
      * @param tasks
      */
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        /**
          * 首先将每个 executor 要执行的 task 信息,统一进行序列化操作
          */
        ...
        if (serializedTask.limit >= maxRpcMessageSize) {
          ...
          /**
            * 向 executor 发送 LaunchTask 消息,来再 executor 上启动task
            */
