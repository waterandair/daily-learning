...
/**
  * BlockManagerMasterEndpoint 就是负责维护哥哥executor 的 BlockManager
  */
/**
...
class BlockManagerMasterEndpoint(
  ...
  /**
    * 这个map , 映射了 blockManager id -> blockManagerInfo,
    * BlockManagerMaster 要负责维护每个 BlockManager 的 BlockManagerInfo
    */
  // Mapping from block manager id to the block manager's information.
  private val blockManagerInfo = new mutable.HashMap[BlockManagerId, BlockManagerInfo]

  /**
    * 这个 映射, 映射了每个 executorId 到 BlockManagerId
    * 也就是说, 每个 executor 是与一个 BlockManager 相关联的
    */
  ...
  private def removeBlockManager(blockManagerId: BlockManagerId) {
    ...
    /**
      * 从 BlockManagerIdByExecutor map 中移除 executorId 对应的 BlockManagerInfo
      */
    ...
    /**
      * 遍历 BlockManagerInfo 内部所有的 block 块的 blockId
      *
      */
    while (iterator.hasNext) {
      /**
        * 清空 BlockManagerInfo 内部的 Block 的 BlockStatus 信息
        */
  ...
  /**
    * 获取 executorId 对应的 BlockManagerInfo,对其调用 removeBlockManager() 犯法
    */
  ...
  private def register(
    ...
    /**
      * 首先判断,如果 本地 HashMap 中没有指定的 BlockManagerId ,说明从来没有注册过
      * 那么才会往下走,继续去注册
      */
    if (!blockManagerInfo.contains(id)) {
      /**
        * 根据 BlockManager 对应的 Executorid找到对应的 BlockManagerInfo
        * 这里做了一个安全判断
        * 因为,如果 BlockManagerInfo map 里没有 BlockManagerid
        * 那么 同步的BlockManagerIdByExecutor map 里, 也必须没有
        * 所以这里要判断一下,如果 BlockManagerIdByExecutor map 里有BlockManagerId, 那么
        * 做一下清理
        */
      blockManagerIdByExecutor.get(id.executorId) match {
        case Some(oldId) =>
          ...
          // 从内存中移除 executorId 关联的 BlockManagerInfo
      ...
      /**
        * 往 BlockManagerIdByExecutor map 中保存一份 ExecutorId 到 BlockManagerId 的映射
        */
      blockManagerIdByExecutor(id.executorId) = id

      /**
        * 为 BlockManagerId 创建一份 BlockManagerInfo
        * 并为 BlockManagerInfo map 中, 保存一份 BlockManagerId 到 BlockManagerInfo 的映射
        */
  ...
  /**
    * 更新 BlockInfo
    * 也就说, 每个 BlockManager上, 如果block 发生了变化,那么都要发送 updateBlockInfo 请求 BlockManagerMaster
    * 这里,进行 BlockInfo 的更新
    */
  private def updateBlockInfo(
    ...
    /**
      * 调用 BlockManager 的 BlockManagerInfo 的 updateBlockInfo() 方法, 更新 block 信息
      */
    blockManagerInfo(blockManagerId).updateBlockInfo(blockId, storageLevel, memSize, diskSize)

    /**
      * 每一个 block 可能会在多个 BlockManager 上面
      * 如果将 StorageLevel 设置成 带 _2 这种,那么就需要将 block 复制一份,放到其他 BlockManager
      *
      * blockLocations map, 就是保存了每个 BlockId 对应的 BlockManagerId 的 set 集合
      * 所以这里,会更新 blockLocations 中的信息,以为是用 set 存储 BlockManagerId, 因此自动去重了
      */
...
/**
  * 每个 BlockManager 的 BlockManagerInfo, 就是 BlockManager 的元数据
  */
private[spark] class BlockManagerInfo(
  ...
  /**
    * BlockManagerInfo 管理了 每个 BlockManager 内部的 BlockId -< BlockStatus 的映射
    */
  ...
  def updateBlockInfo(
    ...
    if (blockExists) {
      ...
      /**
        * 判断如果 storageLevel 是使用内存,那么就给剩余内存数量加上当前的内存量
        */
    ...
    /**
      * 给 Block 创建一份 BLockStatus,然后根据其持久化级别,对相应的内存资源进行计算
      */
    if (storageLevel.isValid) {
      ...
      /**
        * 如果 StorageLevel 是非法的, 而且之前保存过这个 blockId
        * 那么就将 BlockId 存内存中
        */
