...
/**
  * 在 TaskSchedulerImpl 中,对一个单独的 TaskSet 的任务进行调度.这个类负责追踪每一个 task, 如果 task 失败的话,会负责
  * 重试 task, 直到超过重试的次数限制,并且会通过延迟调度,为这个 TaskSet 处理本地化调度机制.它的主要接口是 resourceOffer,
  * 在这个接口中,TaskSet 会希望在一个节点上运行一个任务,并且接受任务的状态改变消息,来知道它负责的task的状态改变了
  *
  */
/**
...
private[spark] class TaskSetManager(
  ...
  /**
    * 这个方法的大致意思
    * 就是说,去判断这个 executor 在这个本地化级别,之前的等待时间是多少
    * 如果说,本地化级别的等待时间在一定范围内,那么就认为 task 使用本地化级别可以再 executor 上启动
    */
